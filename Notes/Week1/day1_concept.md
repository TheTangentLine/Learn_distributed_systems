...
**Step 2: Create** `server.go` This server simulates a "flaky" network. It randomly sleeps (latency) or crashes (reliability issues).
...
func handler(w http.ResponseWriter, r _http.Request) {
    // Simulate Network Latency (0 to 2 seconds)
    delay := time.Duration(rand.Intn(2000)) _ time.Millisecond
    time.Sleep(delay)

    // Simulate Network Failure (20% chance of dropping the request)
    if rand.Float32() < 0.2 {
...
**Step 3: Create** `client.go` This client assumes the network is perfect. It has no timeout and no retry logic.
...
func main() {
    ...
    if err != nil {
        panic(err) // Crashes immediately on error
    }
...
**Review Question**
Look at the `client.go` code. If `http.Get` hangs for 30 seconds because the server is slow, what happens to your application thread? (Hint: This is why timeouts are critical).
